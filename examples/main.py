...
PokemonCreate: PY_MODEL = sa2pydantic(
    Pokemon,
    exclude_fields=["id"],
    exclude_rel=True,
...
PokemonOut: PY_MODEL = sa2pydantic(
    Pokemon,
    name_call=lambda sa: f"{sa.__name__.title()}Out",
)


TrainerCreate: PY_MODEL = sa2pydantic(
    Trainer,
    exclude_fields=["id"],
    exclude_rel=True,
    name_call=lambda sa: f"{sa.__name__.title()}Create",
...
TrainerOut: PY_MODEL = sa2pydantic(
    Trainer,
    name_call=lambda sa: f"{sa.__name__.title()}Out",
)


PokemonOutOptional: PY_MODEL = sa2pydantic(
    Pokemon,
...
def describe_model(model: type[BaseModel]):
    print("Describe", model)  # noqa: T201
    for f, i in model.model_fields.items():
        print(f"field {f}: default={i.default}, annotation={i.annotation}")  # noqa: T201
